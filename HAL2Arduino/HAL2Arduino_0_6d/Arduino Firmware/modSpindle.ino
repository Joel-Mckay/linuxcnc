#define useSpindle true // load spindle control code.
  ...
  const byte spindleEnablePin=35;
  const byte spindleBrakePin=49; //logic dump

//use power mosfets
  const byte spindleDirPin=8; // 2P6P relay, normal CW when off
  const byte spindleThrottlePin=5; // pwm or vfd signal.
// User 5v logic pin outputs
//  const byte spindleDirPin=34;
//  const byte spindleThrottlePin=13; // pwm or vfd signal.
